...
export function BienvenidaEcoFluencer({ abierto, onCerrar }: Props) {
  return (
    <Dialog open={abierto} onOpenChange={(o) => !o && onCerrar()}>
      <DialogContent className="max-w-md overflow-hidden border-2 border-primary/30 bg-card sm:rounded-3xl">
        <DialogHeader className="sr-only">
          <DialogTitle>Bienvenida EcoFluencer</DialogTitle>
          <DialogDescription>Modal de bienvenida al workbook de EcoFluencer.</DialogDescription>
        </DialogHeader>

        <div className="flex flex-col items-center gap-5 px-1 pb-2 pt-4 text-center">
          <img
            src={personaje}
            alt="Personaje EcoFluencer"
